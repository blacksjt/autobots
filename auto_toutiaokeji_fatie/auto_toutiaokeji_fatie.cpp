...
auto_toutiaokeji_fatie::auto_toutiaokeji_fatie(QWidget *parent)
    : QMainWindow(parent),
...
auto_toutiaokeji_fatie::~auto_toutiaokeji_fatie()
{

}

void auto_toutiaokeji_fatie::onStart()
{
  // 检查输入
  if (!CheckInput())
  {
...
void auto_toutiaokeji_fatie::onPause()
{
  control_status = false;
  ui.lineEdit_msg->setText(QStringLiteral("已停止"));
}

bool auto_toutiaokeji_fatie::DoWork()
{
...
bool auto_toutiaokeji_fatie::DoPostFatie(const QString& content)
{
  ...
  header_list.push_back(HttpParamItem("Origin", "http://mini.eastday.com"));
  ...
  post_data.push_back(HttpParamItem("client_id", "cyrQbYhjk"));
...
bool auto_toutiaokeji_fatie::GetContent()
{
...
bool auto_toutiaokeji_fatie::RequestForLogin()
{
  //qint64 ntime = QDateTime::currentMSecsSinceEpoch();
  //ntime = ntime*10 + 1;
  //QString time = QString::number(ntime);
  QString str_url1 = "http://changyan.sohu.com/api/2/login/passport?client_id=cyrQbYhjk&platform_id=2";
...
void auto_toutiaokeji_fatie::CodeCheckForRenren()
{

}

bool auto_toutiaokeji_fatie::AuthorBySina(const QString& name, const QString& password)
{
  //1.检验验证码
...
void auto_toutiaokeji_fatie::onAddCommentID()
{
...
void auto_toutiaokeji_fatie::UpdateData()
{
...
void auto_toutiaokeji_fatie::initialize()
{
  ...
  m_appkey= "6fihUX";
  m_state = "O2CRSFfeada19cf19e43bc85a3095eb924ef17";
}

void auto_toutiaokeji_fatie::onActFromTxt()
{
...
void auto_toutiaokeji_fatie::Logout()
{
...
void auto_toutiaokeji_fatie::onActImportComment()
{
  QString path = QFileDialog::getOpenFileName(this, QStringLiteral("请选择文件路径"), "", QStringLiteral("Txt(*.txt)")); 
  if (path.isEmpty())  //判断用户是否选择了文件
  {
...
void auto_toutiaokeji_fatie::onActClearComments()
{
  ui.listWidget_comment_id->clear();
  m_comment_list.clear();
  m_comment_item_list.clear();
...
void auto_toutiaokeji_fatie::onActClearAccounts()
{
  ui.tableWidget_account_id->clearContents();
  ui.tableWidget_account_id->setRowCount(0);
  m_account_order = 0;
...
bool auto_toutiaokeji_fatie::CheckInput()
{
...
void auto_toutiaokeji_fatie::FatieSucceed(const QString& id)
{
  m_comment_item_list[m_comment_order]->setCheckState(Qt::Checked);
...
bool auto_toutiaokeji_fatie::GetCsrfToken(const QByteArray& rp_data)
{
...
bool auto_toutiaokeji_fatie::ProcessRedirectSSL(const QString& str)
{
  HttpParamList header_list;
  header_list.push_back(HttpParamItem("Connection","Keep-Alive"));
  header_list.push_back(HttpParamItem("Accept-Encoding","deflate"));
...
bool auto_toutiaokeji_fatie::GetPostId(const QByteArray& arr)
{
...
bool auto_toutiaokeji_fatie::ProcessRedirectLoginGet(const QString& str)
{
  HttpParamList header_list;
  header_list.push_back(HttpParamItem("Connection","Keep-Alive"));
  //header_list.push_back(HttpParamItem("Accept-Encoding","deflate"));
...
bool auto_toutiaokeji_fatie::ProcessRedirectLoginGet2(const QString& str)
{
  HttpParamList header_list;
...
bool auto_toutiaokeji_fatie::GetFatieStatus(const QByteArray& byte_arr)
{
...
bool auto_toutiaokeji_fatie::PreLoginSina(const QString& name, SinaData& data, QString& vcode, QString& code_sign)
{
...
bool auto_toutiaokeji_fatie::GetPreLoginResult(const QByteArray& str, SinaData& data)
{
...
int auto_toutiaokeji_fatie::LoginSina(SinaData& sina_data,const 
...
int auto_toutiaokeji_fatie::GetLoginResult(const QByteArray& str, SinaData& data)
  {
...
bool auto_toutiaokeji_fatie::AuthorizeSina(const SinaData& sina_data)
{
...
bool auto_toutiaokeji_fatie::AuthorizeSina2(const QByteArray& rp,const SinaData& sina_data)
{
...
bool auto_toutiaokeji_fatie::GetQrid(const QByteArray& rp_data, QString& qrid)
{
...
void auto_toutiaokeji_fatie::onDeveliver()
{
  control_status = true;
...
bool auto_toutiaokeji_fatie::ProcessRedirectGet2(const QString& str)
{
  HttpParamList header_list;
  header_list.push_back(HttpParamItem("Connection","Keep-Alive"));
  //header_list.push_back(HttpParamItem("Accept-Encoding","deflate"));
  ...
  if (n == 302 || n == 301)
  {
    ...
    reply->deleteLater();
    return ProcessRedirectGet3(str2);
  }
  else
  {
    reply->deleteLater();
...
bool auto_toutiaokeji_fatie::ProcessRedirectGet3(const QString& str)
{
  HttpParamList header_list;
  header_list.push_back(HttpParamItem("Accept","text/html, application/xhtml+xml, */*"));
  header_list.push_back(HttpParamItem("Connection","Keep-Alive"));
...
bool auto_toutiaokeji_fatie::DoSupport()
{
  ...
  for (size_t i = 0; i < commentlist.size(); ++i)
  {
    ...
    QString str_url1 = QString("http://changyan.sohu.com/api/2/comment/action?callback=fn&action_type=1&comment_id=%1&client_id=%2&topic_id=%3&_=%4").arg(
      commentlist[i], "cyrQbYhjk", m_news_id, GetTimeStr());
    QUrl url1;
    url1.setUrl(str_url1);
...
QString auto_toutiaokeji_fatie::GetTimeStr()
{
...
bool auto_toutiaokeji_fatie::GetVerifyToken(const QByteArray& arr, QString& uid, QString& verifytoken)
{
  HtmlParseUtils html_parse(arr.data(), arr.length());
