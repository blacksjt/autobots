...
//	header_list.push_back(HttpParamItem("X-CSRFToken", m_csrf_token));
	...
	post_data.push_back(HttpParamItem("forum_id", m_group_id));
	...
	post_data.push_back(HttpParamItem("thread_id", m_news_id));
...
bool autobots_toutiao::GetFatieStatus(const QByteArray& byte_arr)
{
	...
	if (json_error.error == QJsonParseError::NoError)
	{
		if (parse_doucment.isObject())
		{
			...
			{
				QJsonValue name_value = obj.take("comment_id");
				if (name_value.isDouble())
				{
					double id = name_value.toDouble();
					QString comment_id = QString::number(id, 'f', 0);

					if (!comment_id.isEmpty())
					{
						FatieSucceed("succeed", comment_id);
						return true;
					}
				}
			}
