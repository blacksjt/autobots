...
void autobots_toutiao::onStart()
{
  ...
  {
    if (!control_status || m_url_list.isEmpty())
    {
      break;
    }
	if (network != NULL)
...
{
    QString str_url1 = "http://www.toutiao.com/group/repin/";
	...
	for (int i = 0; i < m_url_list.size(); ++i)
	{
		...
		header_list.push_back(HttpParamItem("X-CSRFToken", "undefine"));
		header_list.push_back(HttpParamItem("Host", "www.toutiao.com"));
		header_list.push_back(HttpParamItem("Referer", m_url_list[i]._url));
		...
		post_data.push_back(HttpParamItem("group_id", m_url_list[i]._group_id));
		post_data.push_back(HttpParamItem("item_id", m_url_list[i]._item_id));
		...
		{
			QString msg = QStringLiteral("收藏成功:") + m_url_list[i]._url;
...
//  m_base_count = ui.spinBox_min_count->value();
  ...
  for (int i = 0 ; i < n; ++i)
  {
	...
	QString str = ui.treeWidget_comment_id->topLevelItem(i)->text(0);
	...
	UrlParam param(strlist.at(0), strlist.at(1), strlist.at(2));
	m_url_list.push_back(param);
...
{
	QString str_url1 = QString("https://www.toutiao.com/auth/connect/?type=sso&platform=sina_weibo&next=https://sso.toutiao.com/auth/login_success/?service=http://www.toutiao.com/");
