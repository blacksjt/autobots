...
bool autobots_toutiao::DoPostFatie(const QString& content)
{
  ...
  // http://wenda.toutiao.com/wenda/web/commit/postanswer/
  QString str_url1 = "http://wenda.toutiao.com/wenda/web/commit/postanswer/";
...
//  header_list.push_back(HttpParamItem("Accept-Encoding","deflate"));
  ...
  QString c_temp = "<p>" + content + "</p>";
  post_data.push_back(HttpParamItem("content", c_temp));
  post_data.push_back(HttpParamItem("forward_pgc", "0"));
  post_data.push_back(HttpParamItem("qid", m_news_id));
...
//   }
  ...
  {
    QMessageBox::critical(this, QStringLiteral("提示"), QStringLiteral("没有评论的内容")); 
    return false;
  }
...
int autobots_toutiao::ProcessRedirectLoginGetTemp2(const QString& str)
{
  ...
  QString s_url = "http://www.toutiao.com/auth/connected/?state=renren_sns__0____toutiao____2__0__24&" + t;
...
bool autobots_toutiao::GetFatieStatus(const QByteArray& byte_arr)
{
  ...
  {  
    ...
    {  
      ...
      if(obj.contains("err_tips"))  
      {  
        QJsonValue name_value = obj.take("err_tips");
        if(name_value.isString())
        {
          ...
          if (str_value == "success")
          {
            ...
            QString comment_id ;
            QString comment_str;
            if(obj.contains("ansid"))  
            {  
              QJsonValue comment_obj = obj.take("ansid");
              if(comment_obj.isString())
              {
                comment_id = comment_obj.toString();
              }
            }
