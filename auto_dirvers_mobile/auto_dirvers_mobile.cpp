...
QString auto_dirvers_fatie::calKey(const QString & _id, const QString & _password)
{
    /*paramGetUserInfo = MD5Util.getMD5String(paramUserInfo.getUser_Name() + paramUserInfo.getUser_Pwd());
    paramGetUserInfo = MD5Util.getMD5String(paramGetUserInfo.substring(0, paramGetUserInfo.length() - 5));
    paramGetUserInfo = MD5Util.getMD5String(paramGetUserInfo.substring(5, paramGetUserInfo.length()));
    paramContext.put("skey", paramGetUserInfo.substring(6, paramGetUserInfo.length()));*/

    std::string ret = "";
    ret = md5(_id.toStdString() + _password.toStdString());
    ret = md5(ret.substr(0, ret.length() - 5));
    ret = md5(ret.substr(5, ret.length()));
    ret = ret.substr(6, ret.length());
