...
autobots_weibo::autobots_weibo(QWidget *parent)
    : QMainWindow(parent),
    m_account_order(0)
{
    ui.setupUi(this);
...
autobots_weibo::~autobots_weibo()
{

}

void autobots_weibo::onStart()
{
  // 检查输入
  if (!CheckInput())
  {
...
void autobots_weibo::onPause()
{
  control_status = false;
  ui.lineEdit_msg->setText(QStringLiteral("已停止"));
}

bool autobots_weibo::DoWork()
{
  //http://weibo.com/aj/v6/mblog/forward?ajwvr=6&domain=100306&__rnd=1458012358844
  QString str_url1 = "http://weibo.com/aj/v6/mblog/forward?ajwvr=6&domain=100306&__rnd="+GetTimeStr();
  QUrl url1;
  ...
  for (size_t i = 0; i < m_comment_list.size(); ++i)
  {
    ...
    post_data.push_back(HttpParamItem("appkey",""));
    post_data.push_back(HttpParamItem("is_comment","1"));
    post_data.push_back(HttpParamItem("location", m_location));
    post_data.push_back(HttpParamItem("mark",""));
    post_data.push_back(HttpParamItem("mid",m_mid));
    post_data.push_back(HttpParamItem("module",""));
    post_data.push_back(HttpParamItem("page_module_id",""));
    post_data.push_back(HttpParamItem("pdetail",m_page_id));
    post_data.push_back(HttpParamItem("pic_id",""));
    post_data.push_back(HttpParamItem("pic_src",""));
    post_data.push_back(HttpParamItem("rank","0"));
    post_data.push_back(HttpParamItem("rankid",""));
    post_data.push_back(HttpParamItem("reason",m_comment_list[i]));
    post_data.push_back(HttpParamItem("refer_sort",""));
    post_data.push_back(HttpParamItem("style_type","1"));
...
bool autobots_weibo::GetContent()
{
...
bool autobots_weibo::RequestForLogin()
{
...
bool autobots_weibo::AuthorBySina(const QString& name, const QString& password)
{
  //1.检验验证码
...
void autobots_weibo::UpdateData()
{
...
void autobots_weibo::initialize()
{
...
void autobots_weibo::onActFromTxt()
{
...
void autobots_weibo::onActClearAccounts()
{
  ui.tableWidget_account_id->clearContents();
  ui.tableWidget_account_id->setRowCount(0);
  m_account_order = 0;
...
bool autobots_weibo::CheckInput()
{
...
bool autobots_weibo::GetCsrfToken(const QByteArray& rp_data)
{
...
bool autobots_weibo::ProcessRedirectSSL(const QString& str)
{
  HttpParamList header_list;
  header_list.push_back(HttpParamItem("Connection","Keep-Alive"));
  header_list.push_back(HttpParamItem("Accept-Encoding","deflate"));
...
bool autobots_weibo::ProcessRedirectGet(const QString& str)
{
  HttpParamList header_list;
  header_list.push_back(HttpParamItem("Connection","Keep-Alive"));
  header_list.push_back(HttpParamItem("Accept-Encoding","deflate"));
...
bool autobots_weibo::GetPostId(const QByteArray& arr)
{
...
bool autobots_weibo::ProcessRedirectLoginGet(const QString& str)
{
  HttpParamList header_list;
  header_list.push_back(HttpParamItem("Connection","Keep-Alive"));
  header_list.push_back(HttpParamItem("Accept-Encoding","deflate"));
...
bool autobots_weibo::ProcessRedirectLoginGet2(const QString& str)
{
  HttpParamList header_list;
...
bool autobots_weibo::GetFatieStatus(const QByteArray& byte_arr)
{
...
bool autobots_weibo::PreLoginSina(const QString& name, SinaData& data, QString& vcode,QString& code_sign)
{
...
bool autobots_weibo::GetPreLoginResult(const QByteArray& str, SinaData& data)
{
...
int autobots_weibo::LoginSina(SinaData& sina_data,const QString& name, 
...
int autobots_weibo::GetLoginResult(const QByteArray& str, SinaData& data)
{
...
bool autobots_weibo::AuthorizeSina(const SinaData& sina_data)
{
...
bool autobots_weibo::GetQrid(const QByteArray& rp_data, QString& qrid)
{
...
QString autobots_weibo::GetTimeStr()
{
...
bool autobots_weibo::GetVoteResult(const QByteArray& data)
{
  bool res = false;
  QJsonParseError json_error;
...
bool autobots_weibo::GetAuthorResult(const QByteArray& data)
{
  return true;
}


void autobots_weibo::switchIP()
{
  //执行完即返回0  
...
void autobots_weibo::onAddCommentID()
{
...
void autobots_weibo::AddToTree(const QString& text)
{
...
void autobots_weibo::onActImportComment()
{
  QString path = QFileDialog::getOpenFileName(this, QStringLiteral("请选择文件路径"), "", QStringLiteral("Txt(*.txt)")); 
  if (path.isEmpty())  //判断用户是否选择了文件
  {
...
void autobots_weibo::onActClearComments()
{
  ui.treeWidget_comment_id->clear();
  m_comment_list.clear();
  m_comment_item_list.clear();
