...
private:
  ...
  bool GetCode(const QByteArray& rp_data, QString& hash_code, QString& public_key);
  bool LoginBili(const QString& name, const QString& password, const QString& hash_code, const QString& public_key);
  void WaitforSeconds(int nseconds);
  bool GetResult(const QByteArray& rp_data);
private:
