...
class auto_cnbeta : public QMainWindow
{
    Q_OBJECT

public:
    auto_cnbeta(QWidget *parent = 0);
    ~auto_cnbeta();
...
private:
  ...
  void UpdateData();
  bool GetContent();
  bool GetCsrfToken(const QByteArray& data);
  bool GetCsrfToken2(const QByteArray& data);
  bool GetToken();
private:
