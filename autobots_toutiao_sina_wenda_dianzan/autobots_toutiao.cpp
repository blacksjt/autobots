...
bool autobots_toutiao::DoAction()
{
  ...
  QString str_url1 = "https://wenda.toutiao.com/wenda/web/commit/digg/";
  ...
  for(int i = 0; i <m_comment_list.size(); ++i)
  {
	...
	header_list.push_back(HttpParamItem("wendacsrftoken", ""));
    ...
    post_data.push_back(HttpParamItem("ansid", str_id));
...
bool autobots_toutiao::GetContent()
{
  ...
  QNetworkReply* rp = network->GetRequest_ssl(url_1, header_list1);
...
bool autobots_toutiao::ProcessRedirectLoginGet2(const QString& str)
{
	...
	{
		reply = network->GetRequest_ssl(QUrl(str), header_list);
	}
	else
	{
		reply = network->GetRequest(QUrl(str), header_list);
	}
