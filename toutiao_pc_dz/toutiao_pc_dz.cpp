...
bool autobots_toutiao::GetValidateCode(const QByteArray& rp_data, QString& vcode, QString& code_sign)
{
	QString str = rp_data;

	int pos1 = str.indexOf("captcha:");
	QString sub_str = str.right(str.length() - pos1);
	pos1 = sub_str.indexOf("'");
	int pos2 = sub_str.indexOf("'", pos1 + 1);

	sub_str = sub_str.mid(pos1 + 1, pos2 - pos1 - 1);

	QByteArray arr = QByteArray::fromBase64(sub_str.toUtf8().data());
